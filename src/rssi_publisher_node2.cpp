...
{
  // ROS initialization
  ros::init(argc, argv, "rssi_publisher_node2");
  ...
  //std::string short_id_2 = argv[2]; // the input short id (second device)
  ...
  std::string topic_name_ = "rssi_val_" + short_id_;
  ros::Publisher rssi_val_pub = nh.advertise<std_msgs::Float64>(topic_name_,100);
  ...
  ros::Rate Loop_Rate(2);  // 5HZ = 0.2s , rate
  while (ros::ok())
  {
